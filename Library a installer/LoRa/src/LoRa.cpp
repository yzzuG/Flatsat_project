...
int LoRaClass::getFrequency()
{
  return ((readRegister(REG_FRF_MSB) * 65536  + readRegister(REG_FRF_MID) * 256 + readRegister(REG_FRF_LSB)) *32) >> 19 ; // En MHz
  //return readRegister(REG_FRF_MSB) << 16 ;
}
...
{
// return readRegister(PA_BOOST);
 return (readRegister(REG_PA_CONFIG)) +2-128;
...
int LoRaClass::getCodingRate4()
{
  ...
  return ((readRegister(REG_MODEM_CONFIG_1))>>1)-48-4;
}
...
{
  return (readRegister(REG_PREAMBLE_MSB) * 256 + readRegister(REG_PREAMBLE_LSB)) ; // En MHz
